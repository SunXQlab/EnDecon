benchmark <- function (sc_data, st_data, number_ct, methods,ground_truth){
  ...
  dir='D:/AA-luluyan-phd/code/benchmark_github/Deconvolution/'
  list.files(dir,pattern = "pipeline.R$")
  for (i in list.files(path=dir, pattern = "pipeline.R$")){
    source(paste0('D:/AA-luluyan-phd/code/benchmark_github/Deconvolution/',i))
  }
  
  source('D:/AA-luluyan-phd/code/benchmark_github/Evaluation/benchmark_performance_fun.R')
  ...
  if (methods == "RCTD"){
    ...
    decon_result <- SPOTlight_pipeline(sc_data = sc_data,
                                       st_data = st_data)
  ...
  return(list(decon_result = decon_result))
}
