...
#'
#' @param ground_truth_mtrx Object of class matrix containing the ground truth composition of each spot.
#' @param deconvoluted_mtrx Object of class matrix with the predicted cell type composition of each spot.
#' @return This function returns a list with RMSE, RMSE_ct, JSD, PCC and PCC_ct.
#'
...
benchmark_performance <- function(ground_truth_mtrx,
  ...
  observed_values <- unlist(lapply(seq_len(ncol(deconvoluted_mtrx)), function(i) deconvoluted_mtrx[,i]))+1e-12
  expected_values <- unlist(lapply(seq_len(ncol(ground_truth_mtrx)), function(i) ground_truth_mtrx[,i]))+1e-12
  ...
  return(list(RMSE = RMSE, RMSE_perct = RMSE_ct, JSD0=JSD,
